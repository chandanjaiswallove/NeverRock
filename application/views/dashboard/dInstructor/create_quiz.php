...
<main class="bg-transparent">
        ...
        <div class="container-fluid-2">
            <div
                ...
                <div class="tab-links flex flex-wrap mb-10px lg:mb-50px rounded gap-10px"  data-aos="fade-up" >
                        <button class="is-checked relative py-10px px-5 md:py-15px lg:px-10 font-bold uppercase text-sm lg:text-base text-blackColor bg-whiteColor shadow-overview-button dark:bg-whiteColor-dark dark:text-blackColor-dark before:w-0 before:h-0.5 before:absolute before:-bottom-0.5 lg:before:bottom-0 before:left-0 before:bg-primaryColor hover:before:w-full before:transition-all before:duration-300 whitespace-nowrap active">
                            QUIZ DETAILS
                        </button>
                    </div>

                <!-- QUIZ INFO FORM -->
                <form id="quizForm" data-aos="fade-up"
                    ...
                    <!-- Basic Info -->
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3"  data-aos="fade-up" >
                        <div>
                    ...
                    <div class=""  data-aos="fade-up">
                        ...
                        <textarea id="quizDesc" placeholder="Enter short description"
                            class="w-full py-10px px-5 border-2 border-borderColor dark:border-borderColor-dark rounded-md bg-whiteColor dark:bg-whiteColor-dark"></textarea>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4"  data-aos="fade-up" >
                        ...
                        <div>
                            ...
                            <input type="number" id="timeLimit" placeholder="e.g. 30"
                                class="w-full py-10px px-5 border-2 border-borderColor dark:border-borderColor-dark rounded-md bg-whiteColor dark:bg-whiteColor-dark">
                        </div>
                        <div>
                    ...
                    <!-- Add Quiz Buttons -->
                    <div class="mb-6 mt-6" data-aos="fade-up">
                        <button id="addQuizBtnTop" type="button"
                            class="w-full py-10px px-5 md:py-15px lg:px-10 font-bold uppercase text-sm lg:text-base text-blackColor bg-whiteColor shadow-overview-button dark:bg-whiteColor-dark dark:text-blackColor-dark">
                            + Add Question
                        </button>
                    </div>
                    ...
                    <!-- Add new question button below all quizzes -->
                    <div class="mt-4"  data-aos="fade-up">
                        <button type="button" id="addQuizBtnBottom"
                            class="w-full py-2 px-5 bg-secondaryColor text-whiteColor rounded hover:bg-primaryColor-dark font-semibold">
                            + Add Question
                        </button>
                    </div>

                    <!-- Save All Quiz Button -->
                    <div class="mt-15px"  data-aos="fade-up">
                        <button type="submit"
    ...
    <script>
        ...
        const addQuizBtnTop = document.getElementById('addQuizBtnTop');
        const addQuizBtnBottom = document.getElementById('addQuizBtnBottom');
        let quizCount = 0;
        ...
        function addQuizField() {
            ...
            div.className = 'quiz-item space-y-3 mb-4 border-b border-borderColor dark:border-borderColor-dark pb-4';
            div.innerHTML = `
            ...
            <div class="text-center">
                <button type="button" class="remove-quiz text-sm font-semibold text-primaryColor bg-whitegrey3 px-6 py-5px mb-5 rounded-standard inline-block">Remove Question</button>
            </div>
        `;
            quizContainer.appendChild(div);
        }

        addQuizField(); // Add one by default
        addQuizBtnTop.addEventListener('click', addQuizField);
        addQuizBtnBottom.addEventListener('click', addQuizField);
        ...
        function updateQuizNumbers() {
            ...
            allQuestions.forEach((item, index) => {
                ...
                radios.forEach(r => r.setAttribute('name', `correctAnswer${index + 1}`));
            });
        ...
        document.getElementById('quizForm').addEventListener('submit', function (e) {
            ...
            for (let i = 0; i < allQuestions.length; i++) {
                ...
                const options = Array.from(qItem.querySelectorAll('.option-input')).map(opt => opt.value);
                const correctRadio = qItem.querySelector('input[type="radio"]:checked');
                const correctAnswer = correctRadio ? parseInt(correctRadio.value) : null;

                if (!questionText || options.includes('') || correctAnswer === null) {
                    alert(`Please fill all fields and select correct answer for Question ${i + 1}`);
                    return;
                }
            ...
            // send to backend via fetch('/api/quiz', { method:'POST', body:JSON.stringify(finalQuiz) })
        });
    </script>
