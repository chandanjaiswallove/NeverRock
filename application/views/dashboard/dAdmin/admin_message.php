...
<main class="bg-transparent">
    ...
    <script>
        ...
        function calculateFinalPrice() {
            // Remove ₹ or non-digit for regular
            const regular = parseFloat(regularInput.value.replace(/[^\d.]/g, "")) || 0;

            // Remove non-digit for discount
            ...
            // Update Discount display only if user finished typing
            if (discount > 0) {
                discountInput.value = `Discount Applied ${discount}%`;
            } else {
                discountInput.value = "";
            }
        ...
        // Listen to input
        regularInput.addEventListener("input", calculateFinalPrice);
        discountInput.addEventListener("input", calculateFinalPrice);

        // Optional: select all on focus
        regularInput.addEventListener("focus", () => { regularInput.select(); });
        discountInput.addEventListener("focus", () => { discountInput.select(); });
    </script>
